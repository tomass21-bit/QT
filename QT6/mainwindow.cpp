...
void MainWindow::StartRace(void){
    ...
    if(ui->rb_qtConcur->isChecked()){
        auto races1 =[&](){ concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value()); };
        auto races2 =[&](){ concurRace2->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value()); };
         QtConcurrent::run(races1).then(races2);
